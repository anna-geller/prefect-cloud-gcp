"""
prefect deployment build flows/ingestion_bigquery_taxi_data.py:taxi_data -n yellow -q default -a
prefect deployment build flows/ingestion_bigquery_taxi_data.py:taxi_data -n yellow -q default -a --param table_name=green_tripdata
prefect deployment build flows/ingestion_bigquery_taxi_data.py:ingestion_bigquery_taxi_data -n yellow -q default -a
prefect deployment build flows/ingestion_bigquery_taxi_data.py:ingestion_bigquery_taxi_data -n yellow -q default -a --param table_name=green_tripdata
"""
...
@flow(task_runner=SequentialTaskRunner())
def ingestion_bigquery_taxi_data(
    ...
    for file in files:
        if (
            ...
            df = extract.with_options(name=f"🗂️extract_{file}").submit(file)
